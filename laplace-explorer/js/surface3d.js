...
const Surface3D = (() => {
  ...
  function autoClip(){
    ...
    const K = Math.abs(Model.state.K);
    const bw = Model.bodeWindow();
    const vals = [];
    for (let i = 0; i <= 300; i++){
      const w = bw.log ? bw.min * Math.pow(10, (i / 300) * bw.dec) : bw.max * i / 300;
      const [nr, ni] = hornerC(num, 0, w);
      const [dr, di] = hornerC(den, 0, w);
      const dm = Math.hypot(dr, di);
      if (dm === 0) continue;
      const m = K * Math.hypot(nr, ni) / dm;
  ...
  function updatePane(p){
    const { num, den } = Model.polys();
    const K = Model.state.K, Kabs = Math.abs(K);
    ...
    const aliased = isPhase && Model.state.delay > 0 && (() => {
    ...
    const valAt = isPhase
      ? (sigma, omega) => {
          const [nr, ni] = hornerC(num, sigma, omega);
          const [dr, di] = hornerC(den, sigma, omega);
          if ((nr === 0 && ni === 0) || (dr === 0 && di === 0)) return 0;
          return wrapDeg((Math.atan2(ni, nr) - Math.atan2(di, dr)) * 180 / Math.PI
                         + (K < 0 ? 180 : 0) - omega * Td * 180 / Math.PI);
        }
      : (sigma, omega) => {
          const [nr, ni] = hornerC(num, sigma, omega);
          const [dr, di] = hornerC(den, sigma, omega);
          const dm = Math.hypot(dr, di);
          let m = dm === 0 ? Infinity : Kabs * Math.hypot(nr, ni) / dm;
          if (Td > 0) m *= Math.exp(-sigma * Td);
          if (!isDb) return m;
  ...
  function updateMarkers(p, xOf, zOf, yOf, valAt){
    ...
    const bw = Model.bodeWindow();
    const visible = (Model.state.view.showBode || Model.state.view.ruler.on) &&
                    we !== null && isFinite(we) &&
  ...
  function render(){
    ...
    if (document.activeElement !== clipSlider)
      clipSlider.value = String(Math.round(50 + 50 * Math.log10(Model.state.view.clipFactor || 1)));
