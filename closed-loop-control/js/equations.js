...
const Equations = (() => {
  ...
  const titleToggle = document.getElementById('eq-title-toggle');
  ...
  const functionSel = document.getElementById('eq-function');
  const linearWarning = strip.querySelector('[data-linear-warning]');
  functionSel.addEventListener('change', () => Model.setView({ equationFunction: functionSel.value }));

  // Le pliage appartient uniquement au titre. La ligne contient plusieurs
  // commandes rapprochées : lui donner un gestionnaire global rendait un clic
  // sur « Transformée de Fourier » ambigu dès que le pointeur dérivait.
  ...
  function render(){
    ...
    titleToggle.title = eqToggle.title;
    titleToggle.setAttribute('aria-label', eqToggle.title);
    titleToggle.setAttribute('aria-expanded', String(!collapsed));
    fitChk.checked = Model.state.view.eqFit;
    const which = ['P', 'C', 'S', 'L', 'T'].includes(Model.state.view.equationFunction)
      ? Model.state.view.equationFunction : 'L';
    functionSel.value = which;
    linearWarning.classList.toggle('hidden', !Model.state.saturation.enabled);
    freqLabel.classList.remove('hidden');
    const freq = Model.state.view.eqFreq;
    freqChk.checked = freq;
    rowFreq.classList.toggle('hidden', !freq);
    if (collapsed) return;
    const { fact, expd, ode, mod, arg } = TeX.all(which);
